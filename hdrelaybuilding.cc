...
main (int argc, char *argv[])
{
  ...
  b->SetBoundaries (Box (-1.5, 1.5, -1.5, 1.5, -1.5, 1.5));
  ...
  wifiChannel.AddPropagationLoss ("ns3::OhBuildingsPropagationLossModel","InternalWallLoss",DoubleValue(10.0),"ShadowSigmaIndoor",DoubleValue(12.0));
  ...
  positionAlloc->Add (Vector (0.0, +1.0, +1.0));   /* PCP/AP */
  positionAlloc->Add (Vector (0.0, -1.0, +1.0));   /* RDS */
  positionAlloc->Add (Vector (-1.0, 0.0, +1.0));   /* West STA */
  positionAlloc->Add (Vector (+1.0, 0.0, +1.0));   /* East STA */
  ...
  Simulator::Run ();
  Simulator::Destroy ();
  flowMonitor->SerializeToXmlFile("hdrelay.xml", true, true);
