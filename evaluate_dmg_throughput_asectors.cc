 ...
 * ./waf --run "scratch/evaluate_dmg_throughput --apsectors=8 --stasectors=2"
 *
...
main(int argc, char *argv[])
{
  ...
  uint32_t apsectors = 8;                       /* Number of Access point antena sectors */
  uint32_t stasectors = 2;                      /* Number of station nodes antenna sectors */
  ...
  cmd.AddValue ("apsectors", "Number of access point antenna sectors", apsectors);
  cmd.AddValue ("stasectors", "Number of station nodes antenna sectors", stasectors);
  ...
  cout << "MCS" << '\t' << "Throughput (Mbps)" << '\t' << "Time (s)"<< endl;
  ...
  for (std::list<std::string>::const_iterator iter = dataRateList.begin (); iter != dataRateList.end (); iter++, i++) //MCS
    {
      ...
      /* 8 antenna sectors for access point*/
      wifiPhy.SetAntenna ("ns3::Directional60GhzAntenna",
                          "Sectors", UintegerValue (apsectors),
      ...
      /* 2 antenna sectors for station nodes*/
      wifiPhy.SetAntenna ("ns3::Directional60GhzAntenna",
                          "Sectors", UintegerValue (stasectors),
                          "Antennas", UintegerValue (1));
      ...
      cout << "MCS" << mcs.str () << '\t' << (sink->GetTotalRx () * (double) 8/1e6) / 0.1 << '\t' << '\t' << '\t' << Simulator::Now() << endl;
