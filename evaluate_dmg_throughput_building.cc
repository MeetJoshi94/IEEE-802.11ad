...
main(int argc, char *argv[])
{
  ...
  double InternalWallLoss = 10.0;               /* internal wall loss*/
  bool pcapTracing = false;                     /* PCAP Tracing is enabled or not. */
  uint32_t nfloors = 3;                        /* Number of floors */
  uint32_t nroomsx = 1;                        /* Number of rooms in x */
  uint32_t nroomsy = 2;                        /* Number of rooms in y */
  ...
  cmd.AddValue ("nroomsx", "Number of rooms in x", nroomsx);
  cmd.AddValue ("nroomsy", "Number of rooms in y", nroomsy);
  cmd.AddValue ("InternalWallLoss", "Internal wall loss", InternalWallLoss);
  cmd.Parse (argc, argv);
  ...
  for (std::list<std::string>::const_iterator iter = dataRateList.begin (); iter != dataRateList.end (); iter++, i++) //MCS
    {
      ...
      wifiChannel.AddPropagationLoss ("ns3::OhBuildingsPropagationLossModel","InternalWallLoss",DoubleValue(InternalWallLoss),"ShadowSigmaIndoor",DoubleValue(2.0));

      /* Setting the building*/
      Ptr<Building> b = CreateObject<Building> ();
      b->SetBoundaries (Box (0, 1.5, 0, 1.5, 0, 1.5));
      ...
      cout << "MCS" << mcs.str () << '\t' << (sink->GetTotalRx () * (double) 8/1e6) / 0.1 << endl;
